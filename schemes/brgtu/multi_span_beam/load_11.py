...
def create_beam_load_11(params: dict, beam_nodes: List[Node]):
    a = params["a"]
    b = params["b"]
    c = params["c"]
    d = params["d"]
    ...
    node1 = Node(x=0, y=0)
    node2 = Node(x=a * 0.5, y=0)
    node3 = Node(x=a * 2, y=0)
    node4 = Node(x=a * 3, y=0)
    node5 = Node(x=a * 4, y=0)
    node6 = Node(x=a * 4 + b * 1.5, y=0)
    node7 = Node(x=a * 4 + b * 2, y=0)
    node8 = Node(x=a * 4 + b * 4, y=0)
    node9 = Node(x=(a + b) * 4 + c * 0.5, y=0)
    node10 = Node(x=(a + b) * 4 + c * 2, y=0)
    node11 = Node(x=(a + b) * 4 + c * 4, y=0)
    node12 = Node(x=(a + b + c) * 4 + d * 2, y=0)
    ...
    i = 1
    for node in nodes:
        if node.x not in [n.x for n in beam_nodes]:
            node.name = str(i)
            beam_nodes.append(node)
            i += 1

    beam_nodes.sort(key=lambda node: node.x)
    ...
    return beam_nodes, sections_for_diagram, loads
