...
from core.mechanics.solver import SolvableFrame, multiply_M_frames_by_Simpson, MultiSpanBeam
...
from tasks.brgtu.multi_span_beam.loader import MultiSpanBeamLoader


class BRGTUMultiSpanBeam(TaskPlugin):
    task_id = "multi_span_beam"
    task_name = "Многопролетная балка"
    university = "brgtu"

    def _init_loader(self):
        self.loader = MultiSpanBeamLoader(self.excel_path)
    ...
    def solve(self, cipher: str):
        ...
        layout = keep_only_layout(doc=doc, layout_name_to_keep='Шаблон (многопролетная балка)')
        base_point = [0, 0]

        task_condition_text = (f'{cipher}\n'
                               f'Схема балки - {circuit_number}       Схема нагрузки - {load_number}\n'
                               f"a = {params['a']}м    b = {params['b']}м    c = {params['c']}м    d = {params['d']}м\n"
                               f"P\\H0.5x;1\\H2.0x; = {params['P1']}кН\n"
                               f"P\\H0.5x;2\\H2.0x; = {params['P2']}кН\n"
                               f"P\\H0.5x;3\\H2.0x; = {params['P3']}кН\n"
                               f"q\\H0.5x;1\\H2.0x; = {params['q1']}кН/м\n"
                               f"q\\H0.5x;2\\H2.0x; = {params['q2']}кН/м")
        ...
        print(f"Шифр: {cipher}")
        print(f"Схема балки: {circuit_number}")
        print(f"Схема нагрузки: {load_number}")
        print(f"\nПараметры:")
        print(f"  a = {params['a']} м")
        print(f"  b = {params['b']} м")
        print(f"  c = {params['c']} м")
        print(f"  d = {params['d']} м")
        print(f"  P1 = {params['P1']} кН")
        print(f"  P2 = {params['P2']} кН")
        print(f"  P3 = {params['P3']} кН")
        print(f"  q1 = {params['q1']} кН/м")
        print(f"  q2 = {params['q2']} кН/м")
        print(f"{'=' * 60}")

        def get_circuit_functions(circuit_number):
            # Импортируем модуль динамически
            module = importlib.import_module(f'schemes.brgtu.multi_span_beam.beam_{circuit_number}')
            # module2 = importlib.import_module(f'schemes.brgtu.multi_span_beam.load_{load_number}')

            try:
                # Получаем функции из модуля
                create_beam = getattr(module, f'create_beam_{circuit_number}')
        ...
        create_beam = get_circuit_functions(circuit_number)

        # Создаем главную раму и рисуем ее
        nodes, rods, supports, loads, sections_for_diagram, details = create_beam(params)

        main_beam = MultiSpanBeam(nodes, rods, supports, loads)

        for entity in layout:
            if entity.dxf.layer == "1.Главная рама" and entity.dxftype() == 'VIEWPORT':
                if entity:
                    entity.dxf.view_center_point = (main_beam.geometrical_center()[0], main_beam.geometrical_center()[1], 0.0)
                    entity.dxf.view_height = main_beam.height() + 0.3
        ...
        # eq1 = f'({round_up(delta_11,2)}/EI)·x1 + ({round_up(delta_12,2)}/EI)·x2 +({round_up(delta_13,2)}/EI)·x3 + {round_up(delta_1p,2)}/EI = 0'
        # eq2 = f'({round_up(delta_12,2)}/EI)·x1 + ({round_up(delta_22,2)}/EI)·x2 +({round_up(delta_23,2)}/EI)·x3 + {round_up(delta_2p,2)}/EI = 0'
        # eq3 = f'({round_up(delta_13,2)}/EI)·x1 + ({round_up(delta_23,2)}/EI)·x2 +({round_up(delta_33,2)}/EI)·x3 + {round_up(delta_3p,2)}/EI = 0'
        #
