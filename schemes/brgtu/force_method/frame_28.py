...
def create_frame_28(params: dict):
    l1 = params["l1"]
    l2 = params["l2"]
    h1 = params["h1"]
    h2 = params["h2"]
    ...
    node5 = Node(name='E', x=l2, y=h1 / 2)
    node6 = Node(name='B', x=l2, y=0)
    node7 = Node(name='7', x=l2, y=h1 + h2 * 0.5)
    node8 = Node(name='8', x=l2 + l1 * 0.75 * 0.5, y=h1 + h2 * 0.75)
    node9 = Node(name='T', x=l2 + l1 * 0.75, y=h1 + h2, is_hinge=True)
    node10 = Node(name='10', x=l2 + l1 * 0.75 * 1.5, y=h1 + h2 * 0.75)
    node11 = Node(name='11', x=l2 + l1 * 1.5, y=h1 + h2 / 2)
    node12 = Node(name='12', x=l2 + l1 * 1.5, y=h1)
    node13 = Node(name='S', x=l2 + l1 * 1.5, y=h1 / 2)
    node14 = Node(name='C', x=l2 + l1 * 1.5, y=0)
    node15 = Node(name='15', x=l2 * 2 + l1 * 1.5, y=h1)
    node16 = Node(name='16', x=l2 * 2 + l1 * 1.5, y=h1 / 2)
    node17 = Node(name='D', x=l2 * 2 + l1 * 1.5, y=0)
    node18 = Node(name='18', x=l2 + l1 * 0.75, y=h1 * 0.5)
    ...
    rod6 = Rod(start_node=node4, end_node=node7)
    rod9 = Rod(start_node=node12, end_node=node11)
    rod10 = Rod(start_node=node13, end_node=node12)
    rod11 = Rod(start_node=node14, end_node=node13)
    rod12 = Rod(start_node=node12, end_node=node15, stiffness=i2)
    rod13 = Rod(start_node=node16, end_node=node15)
    rod14 = Rod(start_node=node17, end_node=node16)
    rod15_1 = Rod(start_node=node5, end_node=node18, is_start_hinge=True)
    rod15_2 = Rod(start_node=node18, end_node=node13, is_end_hinge=True)
    ...
    support3 = Support(node=node14, number_of_reactions=2, rotation=90)
    support4 = Support(node=node17, number_of_reactions=2, rotation=90)

    if load_index == 1:
        rod7_1 = Rod(start_node=node7, end_node=node8, stiffness=i3)
        ...
        rods = [rod1, rod2, rod3, rod4, rod5, rod6, rod7_1, rod7_2, rod8_1, rod8_2, rod9, rod10, rod11, rod12, rod13, rod14, rod15_1, rod15_2]

    else:
        rod7 = Rod(start_node=node7, end_node=node9, stiffness=i3)
        rod8 = Rod(start_node=node9, end_node=node11, stiffness=i3)

        load_P1 = Force(name='P', node=node2, value=P, rotation=0)
        load_P2 = Force(name='P', node=node16, value=P, rotation=180)
        ...
        loads = [load_P1, load_P2, load_q1, load_q2]
        nodes = [node1, node2, node3, node4, node5, node6, node7, node9, node11, node12, node13, node14, node15, node16, node17, node18]
        rods = [rod1, rod2, rod3, rod4, rod5, rod6, rod7, rod8, rod9, rod10, rod11, rod12, rod13, rod14, rod15_1, rod15_2]
...
def create_primary_system_28(params: dict):
    l1 = params["l1"]
    l2 = params["l2"]
    h1 = params["h1"]
    h2 = params["h2"]
    ...
    node7 = Node(name='7', x=l2, y=h1 + h2 * 0.5)
    node8 = Node(name='8', x=l2 + l1 * 0.75 * 0.5, y=h1 + h2 * 0.75)
    node9 = Node(name='T', x=l2 + l1 * 0.75, y=h1 + h2)
    ...
    details['equation_of_static_determinacy'] = ' 3 · 4 - 7 = 5'
